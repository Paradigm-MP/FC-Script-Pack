...
function Place:KeyHandle(args)
	...
	--elseif args.key == string.byte("4") then -- credits crate
		--local ray = Physics:Raycast(Camera:GetPosition(), Camera:GetAngle() * Vector3.Forward, 0, 50)
		--local position = ray.position
		--Network:Send("SpawnTierCredit", {pos = position})
	end
end
-----
totalboxes = 0
		...
		if shownearbyloot == true then
			...
			for object in Client:GetStaticObjects() do
				if plypos:Distance(object:GetPosition()) <= range then
					...
					if model == "km03.gamblinghouse.flz/key032_01-f.lod" or model == "geo.cbb.eez/go152-a.lod" or model == "general.blz/go155-a.lod" then --or model == "pickup.boost.cash.eez/pu05-a.lod" then
						if model == "general.blz/go155-a.lod" then -- tier 1
							color = Color(0, 255, 0, alpha)
						elseif model == "km03.gamblinghouse.flz/key032_01-f.lod" then -- tier2
							color = Color(0, 0, 255, alpha)
						elseif model == "geo.cbb.eez/go152-a.lod" then -- tier3
