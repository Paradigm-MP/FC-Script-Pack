...
function Bank:__init()
    ...
    SQL:Execute( "create table if not exists bank_apple (steamid VARCHAR UNIQUE, money INTEGER)" )
end
...
function Bank:Chat(args)
	if args.text == "/money" and args.player:GetValue("NT_TagName") == "[Admin]" then
		args.player:SetMoney(args.player:GetMoney() + 50000)
	elseif args.text == "/all500" and args.player:GetValue("NT_TagName") == "[Admin]" then
		for p in Server:GetPlayers() do
			p:SetMoney(p:GetMoney() + 500)
		end
	elseif args.text == "/money2" and args.player:GetValue("NT_TagName") == "[Admin]" then
		args.player:SetMoney(args.player:GetMoney() + 40000000)
	end
end
